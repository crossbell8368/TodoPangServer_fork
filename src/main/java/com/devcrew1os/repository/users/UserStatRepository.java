...
public interface UserStatRepository extends JpaRepository<UserStat, String> {
    ...
    @Modifying
    @Query("UPDATE UserStat us SET us.lastLogoutAt = :current WHERE us.userId = :userId")
    void updateLastLogout(@Param("userId") String userId, @Param("current")LocalDateTime current);
